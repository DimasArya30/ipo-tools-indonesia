...
import type { IpoStatusFilter } from '../types';

const tabs: { key: IpoStatusFilter; label: string }[] = [
  { key: 'all', label: 'Semua' },
  { key: 'ongoing', label: 'Sedang Berlangsung' },
  { key: 'listed', label: 'Sudah Listing' },
  { key: 'postponed', label: 'Ditunda' },
];

export default function IpoCenter() {
  const { filtered, loading, error, filter, setFilter, sectors, years, summary, retry } = useIpo();
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4 pb-6 anim-fade-up">
      <div>
        <h1 className="text-xl font-extrabold text-slate-900 dark:text-white flex items-center gap-2"><CalendarDays className="w-5 h-5 text-emerald-500" /> IPO Center</h1>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Data IPO Indonesia dari sumber terpercaya</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-4 gap-2.5">
        {[
          { label: 'Total IPO', val: summary.total, color: 'text-slate-900 dark:text-white' },
          { label: 'Sudah Listing', val: summary.listed, color: 'text-emerald-600 dark:text-emerald-400' },
          { label: 'Berlangsung', val: summary.ongoing, color: 'text-amber-600 dark:text-amber-400' },
          { label: 'Ditunda', val: summary.postponed, color: 'text-red-600 dark:text-red-400' },
        ].map((s) => (
          <Card key={s.label} className="text-center">
            {loading ? <div className="h-7 w-10 skeleton mx-auto mb-1" /> : <p className={`text-2xl font-extrabold ${s.color}`}>{s.val}</p>}
      ...
      {showFilter && (
        <Card className="anim-fade-up">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 block">Tahun Listing</label>
      ...
      {error ? (
        <Card className="text-center py-10">
          <p className="text-sm text-red-500 mb-3">Gagal memuat: {error}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{[...Array(6)].map((_, i) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {filtered.map((ipo) => (
            <IpoCard key={ipo._row} ipo={ipo} onClick={() => navigate(`/ipo-center/${ipo.tickerCode}`)} onWatchToggle={() => setWk((k) => k + 1)} />
          ))}
        </div>
      )}
    </div>
